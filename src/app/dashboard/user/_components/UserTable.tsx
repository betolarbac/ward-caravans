...
type UserWithId = RegisterFormData & { id: string };

type UserTablePropsId = {
  data: UserWithId[];
};

export function UserTable({ data }: UserTablePropsId) {
  ...
  const columns: ColumnDef<UserWithId>[] = [
    ...
    },
    {
      ...
      header: "Email",
      cell: ({ row }) => (
        <div className=" font-medium px-2 py-4">{row.getValue("email")}</div>
      ),
    },
    {
      accessorKey: "stakeId",
